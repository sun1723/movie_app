...
export const MovieMore = ({ movie }) => {
  ...
  useEffect(() => {
    if (isOpenPlot) handleOpenPlot();
  }, [isOpenPlot]);

  useEffect(() => {
    if (isOpenDetails) handleOpenDetails();
  }, [isOpenDetails]);
  ...
  return (
    <div className="movie_details">
      <div className="movie_details__tabs">
        <Tab
          title="Plot"
          handleOnclick={() => setIsOpenPlot(true)}
          isOnClick={isOpenPlot}
        />
        <Tab
          title="Details"
          handleOnclick={() => setIsOpenDetails(true)}
          isOnClick={isOpenDetails}
        />
      </div>
